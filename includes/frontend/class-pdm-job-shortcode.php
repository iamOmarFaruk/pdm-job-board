...
class Job_Shortcode {
	...
	private function render_global_modal( string $modal_id, int $post_id, string $job_title, string $thumb_url, array $dept_names, string $location, string $job_type, string $workspace_type, string $salary, string $deadline, string $resps, string $quals, string $benefits, string $contact, string $form_shortcode ) : void {
		...
		if ( $resps ) { $render_card_section( __( 'Responsibilities', 'pdm-job-board' ), $resps, 'fa-solid fa-circle-check' ); }
		if ( $quals ) { $render_card_section( __( 'Qualifications', 'pdm-job-board' ), $quals, 'fa-solid fa-clipboard-list' ); }
		if ( $benefits ) { $render_card_section( __( 'Benefits', 'pdm-job-board' ), $benefits, 'fa-solid fa-gift' ); }
		echo '</div>';
		echo '</div>'; // end left
